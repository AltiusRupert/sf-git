...
function hcPoolConnect (callback){
    ...
    status.hcPool.query(query)
        ...
        .then((result)  => 
            result.rows.forEach(function(row) {
                ...
                myenv.SF_PASSWORD               = row.sf_password__c;
                myenv.SF_API_VERSION            = row.sf_api_version__c;
                myenv.EXCLUDE_METADATA          = row.exclude_metadata__c;
                myenv.GIT_IGNORE                = row.git_ignore__c;
                myenv.REPO_URL                  = row.repo_url__c;
                myenv.REPO_USER_NAME            = row.repo_user_name__c;
                myenv.REPO_USER_EMAIL           = row.repo_user_email__c;
                myenv.REPO_README               = row.repo_readme__c;
    ...
    doAll : function(mainCallback){        
        ...
        const promiseChaining = (req, res, next) => {
            ...
            .then(res => {
                ...
                allenv.forEach(function(env) {
                    myenv = env;
                    status.sfConnection = new jsforce.Connection();
